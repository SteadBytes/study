//! An OS kernel that prints text to the screen with 'complete' error handling
//!
//! Note: This is an improved version of `fledgeos-3` that reports errors to
//! the user.
//!
...
/// Panic handler that reports errors by clearning the screen and printing the
/// error message.
///
/// `rustc` requires programs to have a mechanism to deal with panics -
///  normally provided by the Rust standard library.
...
fn panic(info: &PanicInfo) -> ! {
    ...
    // Spin infinitely to allow the user to read the message and restart the
    // machine manually
    loop {
        hlt();
    }
...
pub extern "C" fn _start() -> ! {
    ...
    // Force a panic to demonstrate the error handling
