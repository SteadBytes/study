...
//!
//! Note: This is a modified version of `render-hex-functional` whereby the
//! `parse` function uses a parallel iterator from `rayon` in order to
//! parellise input parsing.
//!
...
fn parse(input: &str) -> Vec<Operation> {
    input
        ...
        // rayon guarantees that there are never any data races here
        .par_bytes()
        .map(|byte| {
            match byte {
                b'0' => Home,
