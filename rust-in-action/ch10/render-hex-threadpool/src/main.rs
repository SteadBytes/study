...
//! function is implemented using a channel-based task queue and a thread pool
//! to parse input in parallel. In general, the parallel iterator approach
//! taken in `render-hex-parallel-iterator` is preferred where applicable.
//! However it is not always the case that a problem can be expressed by a
//! "tidy" iterator. Despite the `parse` function matching the parallel
//! iterator paradigm well, this implementation provides a good example of the
...
//!
//!
//! ## Thread pool & channel-based task queue
//!
//! Method for parallel processing whereby work is split into individual tasks
//! and sent to worker threads via a channel for execution. Threads communicate
//! via message passing through channels.
//!
//! General approach:
//!
//! 1. Determine an appropraite thread pool size (number of threads to spawn).
//! 2. Split work into multiple *independently executable* tasks.
//! 3. Create **two** channels: one for input, one for results.
//! 4. Send tasks into input channel.
//! 4. Send one "shutdown signal" per thread into input channel.
//!     - Indicates to the receiving thread that there is not more work to be
//!       done and that it should shut down.
//! 6. For each thread in the pool, spawn a process which repeatedly polls the
//!    input channel for new tasks. Once a task is received it executes the
//!    task and send the result to the results channel. If the shutdown signal
//!    is received the loop should exit and the thread shut down.
//! 7. Collect results from the results channel.
//!
//! Note: Tasks will not be performed in the same order as they are placed
//! onto the input queue due to the non-determinism of CPU thread scheduling.
//! If order is important, the tasks should include an indicator (e.g. an
//! index or key) of their ordering in the input which will be included
//! in the result. This value can then be used when collecting results to
//! re-construct the correct ordering.
...
const PARSE_THREAD_POOL_SIZE: usize = 2;
...
/// Messages sent between work/results channels in `parse`.
enum ParseWork {
    /// `(index, byte)` tuple to enable the correct ordering of results.
    Task((usize, u8)),
    /// Marker to indicate that a thread should shut down.
    Finished,
}

/// Parse an input byte into an `Operation`
...
///
/// See `parse_byte` for input -> `Operation` mapping.
fn parse(input: &str) -> Vec<Operation> {
    // Set up two channels to facilitate bi-directional (full duplex)
    // communication. This allows spawned threads to both receive and send
    // messages - in this case receiving `ParseWork` tasks to execute and
    // sending the results of the tasks once complete.
    let (todo_tx, todo_rx) = unbounded();
    let (results_tx, results_rx) = unbounded();

    // Track the total number of input bytes processed in order to create a
    // vector of correct size for the return value
    let mut n_bytes = 0;
    // Fill the task queue with work
    for (i, byte) in input.bytes().enumerate() {
        todo_tx.send(ParseWork::Task((i, byte))).unwrap();
        n_bytes += 1;
    }

    // Send shutdown signal to each thread. Due to the queue semantics, this
    // will be the *last* message received by each thread - ensuring that the
    // thread stops looping for new messages and closes shuts down.
    for _ in 0..PARSE_THREAD_POOL_SIZE {
        todo_tx.send(ParseWork::Finished).unwrap();
    }

    for _ in 0..PARSE_THREAD_POOL_SIZE {
        // Cloning sender/receivers allows them to be shared between threads by
        // creating a new *handle*. The handle can safely be passed into the
        // thread but the underlying channel is unchanged e.g. cloning does
        // not create a separate stream of messages.
        ...
        thread::spawn(move || loop {
            ...
            let result = match task {
                Err(_) => break,
                Ok(ParseWork::Finished) => break,
                Ok(ParseWork::Task((i, byte))) => (i, parse_byte(byte)),
            };
            results.send(result).unwrap();
        });
    }
    // Initialize a complete vector and overwrite with correctly ordered
    // results. This is necessary as parsing using threads causes the results
    // to be returned in a non-deterministic order. Using the original index
    // in `ParseWork::Task` enables the correct ordering to be reconstructed.
    // A `Noop` is used here just in case to ensure that the SVG image is not
    // corrupted by a mistake. All the `Noop`s should be overwritten.
    // TODO: Handle this case explicitly?
    let mut ops = vec![Noop(0); n_bytes];
    for _ in 0..n_bytes {
        let (i, op) = results_rx.recv().unwrap();
        // Insert parsed `Operation` in same position as original input
        ops[i] = op;
    }
    ops
}
